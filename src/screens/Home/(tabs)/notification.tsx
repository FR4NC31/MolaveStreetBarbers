import { StyleSheet, Text, View, TouchableOpacity, ScrollView, Modal, Pressable, ActivityIndicator, useWindowDimensions, Alert } from 'react-native'
          ...
          onPress: async () => {
            try {
              ...
              if (error) {
                ...
                setNotifications(prev => {
                  const updated = prev.filter(notif => !selectedNotifications.has(notif.id));
                  if (onUnreadUpdate) {
                    const newUnreadCount = updated.filter(n => !n.read).length;
                    onUnreadUpdate(newUnreadCount);
                  }
          ...
          {selectionMode ? (
            <>
              ...
              <View style={styles.actionBtnGroup}>
                ...
                <TouchableOpacity
                  style={[styles.iconActionBtn, styles.deleteActionBtn, selectedNotifications.size === 0 && styles.disabledActionBtn]}
                  onPress={handleDeleteSelected}
                  disabled={selectedNotifications.size === 0}
                >
                  <Ionicons name="trash-outline" size={20} color={selectedNotifications.size === 0 ? "#999" : "white"} />
                </TouchableOpacity>
              </View>
            </>
          ) : (
