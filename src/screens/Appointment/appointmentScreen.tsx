...
import ToolTip3 from '../../components/Modals/Tooltips/toolTip3';
...
export default function AppointmentScreen() {
  ...
  // Helper function to get barber ID consistently
  const getBarberId = (barber: BarberItem | null): string => {
    if (!barber) return '';
    // Use the id field if available, otherwise extract from label
    return barber.id || barber.label.replace(' - Barber', '');
  };
    ...
    if (selectedBarber && date) {
      const fetchBookedTimes = async () => {
        try {
          ...
          const queryBarberId = getBarberId(selectedBarber);
  ...
  const handleBookAppointment = async (paymentMethod: string) => {
    ...
    try {
      const result = await insertDropdownSelection({
        barber_id: getBarberId(selectedBarber),
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <EmployeeSchedule barberId={getBarberId(selectedBarber)} />
        ...
        <Services onSelect={handleServiceSelect} barberId={getBarberId(selectedBarber)} />
